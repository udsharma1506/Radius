...
class Property(models.Model):
	...
	def calculate_percentage_validation(self, requirement_budget_max, property_price, percentage_range = .25, requirement_budget_min = None):
		'''
		...
		if requirement_budget_max * Decimal(percentage_range_upper_value) > property_price \
			or requirement_budget_max * Decimal(percentage_range_lower_value) < property_price:
		    return True
		else:
			return False
	...
	def validate_budget(self, requirement):
		...
		if max_budget and min_budget:
			budget_flag = self.calculate_percentage_validation(max_budget, self.price, 0.25, min_budget) #range to evaluate 25 %

		# if max present and not min
		elif max_budget and not min_budget:
			budget_flag = self.calculate_percentage_validation(max_budget, self.price, 0.10) #range to evaluate 10%

		# if min present and not max
		elif not max_budget and min_budget:
			budget_flag = self.calculate_percentage_validation(min_budget, self.price, 0.10)#range to evaluate 10 %

		return budget_flag, .30 # return the flag and match % as 30

	def effective_matching( self, diff_max, diff_min = None):
		'''
	...
	def calculate_range_validation(self, max_bedrooms, min_bedrooms, property_bedrooms):
		'''
		...
		if min_bedrooms:
			...
			if difference_with_max <= 2 or difference_with_min <= 2:
				return True, self.effective_matching(difference_with_max, difference_with_min)
			else:
				return False, 0

		# if any one is absent check for the range and return effective matching % based on value
		elif abs(difference_with_max) <= 2:
				return True, self.effective_matching(difference_with_max)
		else:
			return False, 0
	...
	def validate_bedroom(self, requirement):
		...
		if min_bedrooms and max_bedrooms:
			bedroom_flag, match_percent = self.calculate_range_validation(max_bedrooms, min_bedrooms, self.number_of_bedrooms)
		...
		if not max_bedrooms and min_bedrooms:
			bedroom_flag, match_percent = self.calculate_range_validation(min_bedrooms, max_bedrooms, self.number_of_bedrooms)

		return bedroom_flag, match_percent

	def validate_bathroom(self, requirement):
		'''
		validates the bedroom and bathroom data and return the match percentage
		'''

		min_bathrooms = requirement.get('min_bathrooms')
		max_bathrooms = requirement.get('max_bathrooms')

		# check the if both value present
		if min_bathrooms and max_bathrooms:
			bathroom_flag, match_percent = self.calculate_range_validation(max_bathrooms, min_bathrooms, self.number_of_bathrooms)

		# if max present and min is not present
		if not min_bathrooms and max_bathrooms:
			bathroom_flag, match_percent = self.calculate_range_validation(max_bathrooms, min_bathrooms, self.number_of_bathrooms)
		...
		return bathroom_flag, match_percent
	...
	def validate_property_with_requirement(self, requirement):
		...
		distance_flag, distance_value = self.validate_distance(requirement.get('location'))
		budget_flag, budget_percent_value = self.validate_budget(requirement)
		bedroom_flag, bedroom_match_percent = self.validate_bedroom( requirement)
		bathroom_flag, bathroom_match_percent = self.validate_bathroom( requirement)
