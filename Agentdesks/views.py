...
class GetPropertyfromRequirements(View):
	...
	def get(self, request):
		...
		properties_list = Property.objects.filter(city = city)
		...
		for proprty in properties_list:
			...
			validated, validation_data = proprty.validate_property_with_requirement(requirements_dict)
			...
			if validated:
				...
				if similarity > 0.4:
					proprty_mapper[proprty.name] = similarity
		...
		return JsonResponse(data=sorted_list, status=200)
